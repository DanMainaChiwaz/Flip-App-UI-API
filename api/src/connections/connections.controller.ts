...
export class ConnectionsController {
    ...
    /**
     * return suggested connections basing on Distance and Activities, user filter the suggested connection based on location, activity, skillLevel and playStyle
     * @param req 
     * @param location 
     * @param activity 
     * @param skillLevel 
    ...
    @UseGuards(AuthGuard)
    @Get('suggestions')
    async uggestedConnectionBasedDistanceAndActivites (@Request() req, @Query('location') location: string, @Query('activity') activity: string, @Query('skillLevel') skillLevel: string, @Query('playStyle') playStyle: string) {
        const user = req.user;
